...
| F16 | **Fighting Firefox's own routing** — reopening a navigation to a host Firefox associates with a container, when `privacy.containers.switchDuringNavigation.enabled` is on. Firefox moves the reopen too, so it always wins. | Invisible with the pref off, the default. With it on, a throwaway minted for nothing, and one extra tab left behind per click inside the associated container. |
...
- **Firefox site associations (F16)** — launch with the switching pref on, associate a
  ...
  the request, and its headers, already report Firefox's target container while the tab
  reports its own is a browser fact the mock only repeats. Skipped before 155.
...
- **Real-delay disposal (F10), nightly** — `test/e2e/disposal.realtime.test.ts` takes the
  ...
  run by `npm run test:realtime`; excluded rather than skipped, so `npm test`'s
  skips stay the ones `suite.test.ts` allows.
...
- **Fitness functions** — `test/fitness/`, in `npm test`, milliseconds. Every gate above
  ...
  **mean** anything are still true. Each exists because the property it pins is stated in
  ...
  - **The suite itself** (`suite.test.ts`) — no committed `.only` (which shrinks CI to one
    case and still reports success), skips limited to the documented cases,
