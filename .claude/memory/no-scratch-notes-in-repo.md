...
description: "Investigation notes don't stay in the repo — fold the durable lessons into AGENTS.md/tests and delete the file"
metadata: 
  node_type: memory
  type: feedback
...
conclusions were folded into `AGENTS.md` and the tests themselves, they said:
...
AGENTS.md is loaded every session and tests are executable, so both stay honest.
...
- When an investigation concludes, move each durable lesson to where it will be *found*:
  a browser/API constraint → `AGENTS.md`; a behavioural rule → a real test, named for
