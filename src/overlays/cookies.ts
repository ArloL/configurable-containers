...
export function cookiesFor(url: string, config: Config, matchRule: Deps["matchRule"]): CookieSpec[] {
  ...
  if (!rule) return [];
  // Stryker disable next-line ConditionalExpression,StringLiteral: unreachable from a parsed
  // config — `config/parse` refuses `cookies:` on an `ignore` rule outright, so a matched
  // ignore rule has no cookies to return either way. Kept as the second line of defence,
  // since "ignore means CC does not touch this site" is this function's contract and not
  // the parser's. Narrowed to the two survivors' types; the `true` twin goes with them.
  if (rule.action.kind === "ignore") return [];
  return rule.cookies ?? [];
}
