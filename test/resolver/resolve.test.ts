...
describe("resolve — exemptions & single open", () => {
  ...
  // kind. Comparing kinds alone answers "stay" for every permanent container, so a rule
  // would move a tab once — leaving the default container — and never again.
  ...
  // The common inherit case, and the one where "already there" has no name to compare: a
  // same-tab hop inside the throwaway the initiator is browsing in. Two throwaways are
  // indistinguishable to the resolver, so demanding an identity here would reopen an SSO
  // hop into a second throwaway and log the user out mid-flow.
  ...
  // `stay` for a redirector means *this tab, wherever it is*, not "wherever the click came
  // from". The two only differ in the shim-in-another-container case, where routing the hop
  // spends a reopen on a url the user never sees and the destination is decided one
  // navigation later anyway.
...
describe("resolve — disposable path + continuity", () => {
  ...
  // http is a browsed page like any other. Reading only https as one sends every plain-http
  // tab down the "nobody has browsed here yet" branch, which keeps the throwaway wherever
  // the tab is going — so one LAN box or un-upgraded site follows the user across the web.
  ...
  // `view-source:https://reddit.com/` is what makes the anchor load-bearing: an unanchored
  // test finds "https:" inside it and reads the tab as being ON reddit.com. The tab is
  // showing source, not browsing, so it keeps its container rather than buying a throwaway
  // off a comparison against a url that names no page.
  ...
  // An auto-temp tab sits on about:newtab in a fresh throwaway. The first navigation
  // belongs there: no earlier site to isolate it from, and reopening would strand the tab
  // in a second, pointless throwaway.
  ...
  // A link opened in a NEW tab has no page of its own, so `current` cannot answer the
  // continuity question — but the browser put it in the container of the page the click
  // came from, and that page can. Without this, "open in a new tab" answered differently
  // from clicking in place: a video opened from a YouTube search result landed in its own
  // throwaway, logged out.
...
describe("resolve — a rule outranks continuity", () => {
  it("a matched rule switches container within the same registrable domain", () => {
    ...
    // and the Gmail rule must still take the hop out of the throwaway.
    expect(resolve(
  ...
  it("a group does not override an open rule", () => {
    // The same for groups: youtube.com and mail.google.com share a group and nothing else,
    // so the group is the only thing that could have said "stay".
