...
describe("a background restart — state that must be reconstructed", () => {
  it("resumes the throwaway counter past a container that is still live", async () => {
    ...
    // A second tmp1 would collide by name with the live one, and CC identifies its own
    // throwaways by name: the counter is in memory, the names are not.
  ...
  it("still disposes a throwaway it created before the restart", async () => {
    ...
    // The tab closes after the restart, so the container it names was learned by a
  ...
  it("does not containerize a new-tab page it already containerized", async () => {
    ...
    // The replacement is on about:newtab too, so auto-temp's startup sweep sees it again
    // with no `processed` set left. What keeps the sweep off it is that the tab is no
    // longer in firefox-default.
    expect(containerNames(browser)).toEqual(["tmp1"]);
  });
  ...
  it("leaves a tab that already committed in its right container alone", async () => {
    ...
    // Once a tab's url has committed, tabs.get carries everything the guard state did, so
    // the F2 answer rebuilds in full and a restart is not observable here.
...
describe("a background restart — state that cannot be", () => {
  // reopenedNav holds a tab whose url has not committed, which is why nothing can rebuild
  // it: at restart that tab reads about:blank in some container, and so does a
  // middle-clicked link, which must still be isolated. The requestId separating them
  // exists nowhere else. So the bound is pinned, not the state: one wasted hop, converging.
  ...
  it("resumes the remaining grace of a throwaway emptied before the restart", async () => {
    ...
    // The last tab closes: the grace starts, and the cookies outlive it by five minutes so
    // an undo-close lands back in the same session.
    await browser.closesTab(onlyTabInTheThrowaway);
    ...
    // Still nothing one tick short of the ORIGINAL deadline: measured from when the tab
    // closed, not from when the background came back.
...
describe("restart — a paused container", () => {
  ...
  it("still seeds cookies in a paused container — an overlay never decides a container", async () => {
    ...
    // inside the container the tab is already in, so a paused checkout still gets its
    // consent banner pre-dismissed.
...
// disposer's re-arming sweep would keep running through a closure holding a live port —
// but listeners are the same problem: `mock-port` is additive, like Firefox, so wiring a
// second background ADDS handlers rather than replacing them. Firefox retires the old ones
// by destroying their context; `aSessionPort` models that by gating them.
describe("restart — the dead session stops hearing the browser", () => {
  it("hands a message to the background that is running, not the one that was", async () => {
    ...
    // Answered by the live session, whose armed set is the one the blocking handler
    // consults. An ungated dead listener would claim the reply channel first and arm a set
    // nothing reads: the options page would report routing paused while every navigation
    // carried on being routed.
